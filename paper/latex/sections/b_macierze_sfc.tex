...
Poniższe tabele przenoszą do pracy główne artefakty macierzy SFC wygenerowane z deterministycznego przebiegu modelu \amor{}: ziarno losowania 1, miesiąc 12, commit artefaktu w repozytorium \amor{} \texttt{89528f07}, status \texttt{sfc=pass} oraz \texttt{matrix=pass}. Odpowiadają artefaktom utrzymywanym w repozytorium w \path{docs/sfc-matrix-artifacts/}. Wartości liczbowe w tabeli rekonsyliacyjnej są w jednostkach wewnętrznych modelu i służą tu przede wszystkim jako dowód zerowych reszt księgowych.
...
\bottomrule
\end{tabularx}
\caption{Artefakty macierzy SFC generowane z modelu \amor{}.}
...
\caption{Uzgodnienie stanów, przepływów i korekt wyceny.}
\label{tab:appendix-sfr}\\
\toprule
\textbf{Identyczność} & \textbf{Oczekiwane} & \textbf{Rzeczywiste} & \textbf{Rezyduum} & \textbf{Status} & \textbf{Kanały wykonania i źródło rekonsyliacji} \\
\midrule
\endfirsthead
\toprule
\textbf{Identyczność} & \textbf{Oczekiwane} & \textbf{Rzeczywiste} & \textbf{Rezyduum} & \textbf{Status} & \textbf{Kanały wykonania i źródło rekonsyliacji} \\
...
\caption{Mapowanie wierszy symbolicznych na aktywa i mechanizmy wykonania modelu.}
\label{tab:appendix-mapping}\\
\toprule
\textbf{Macierz} & \textbf{Wiersz} & \textbf{Symbole} & \textbf{Aktywa wykonawcze} & \textbf{Mechanizmy wykonania / uwagi} \\
\midrule
\endfirsthead
\toprule
\textbf{Macierz} & \textbf{Wiersz} & \textbf{Symbole} & \textbf{Aktywa wykonawcze} & \textbf{Mechanizmy wykonania / uwagi} \\
\midrule
\endhead
symbolic-bsm & Cash and public balances & $+H_h$, $+H_f$, $+H_b$, $-H$, $+H_{pub}$, $0$ & Cash and public fund balances (\texttt{Cash}) & Cash covers persisted firm, selected public-fund and NBFI cash plus the currency layer used at paper level. \\
...
symbolic-bsm & Equity & $+E_h$, $-E$, $+E_i$, $+E_f$, $+E_{row}$, $0$ & Equity (\texttt{Equity}) & Foreign equity ownership is holder-resolved; execution evidence records holder-aware equity revaluation. \\
...
symbolic-tfm & Equity revaluation & $+\Delta E_h^v$, $-\Delta E^v$, $+\Delta E_i^v$, $+\Delta E_f^v$, $+\Delta E_{row}^v$, $0$ & Equity (\texttt{Equity}) & Equity revaluation [id: 57]; holder-aware execution evidence. \\
